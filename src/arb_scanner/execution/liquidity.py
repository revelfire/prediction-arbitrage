...
def validate_liquidity(
    ...
    poly_contracts = contracts_from_usd(size_usd, price_poly)
    kalshi_contracts = contracts_from_usd(size_usd, price_kalshi)
    ...
    if poly_slippage > max_slip:
        warnings.append(f"Polymarket slippage {poly_slippage:.4f} exceeds max {max_slip:.4f}")
        passed = False
    if kalshi_slippage > max_slip:
        warnings.append(f"Kalshi slippage {kalshi_slippage:.4f} exceeds max {max_slip:.4f}")
        passed = False
    if poly_depth < min_depth:
        warnings.append(f"Polymarket depth {poly_depth} contracts < min {min_depth}")
        passed = False
    if kalshi_depth < min_depth:
        warnings.append(f"Kalshi depth {kalshi_depth} contracts < min {min_depth}")
        passed = False
    ...
    return LiquidityResult(
        ...
        poly_depth_contracts=poly_depth,
        kalshi_depth_contracts=kalshi_depth,
